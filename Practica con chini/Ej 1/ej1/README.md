...
```c
typedef struct {
    char nombre[18]; //18 bytes de chars + 2 bytes padding
    //0 al 17 inclusive nombre
    uint32_t fuerza; //4 bytes de int + 0 de padding
    //20 al 23 inclucisve fuerza
    uint16_t durabilidad; //2 bytes de int + 2 de padding
    //24 al 25 inclucisve durabilidad
} item_t; //28 bytes
```
...
- Importa que los ítems estén ordenados según el comparador. No hay necesidad
  ...
  //habria que ver si Comparador(inventario[indices[i]] , inventario[indices[i+1]] ) = true 
